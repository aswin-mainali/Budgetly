...
This is a **read-only audit**. Nothing has been fixed. Findings are ordered by
real-world impact. The production build (`npm run build`) **succeeds** — Vite/esbuild
strips types without checking them — so none of these currently block deployment, but
several are genuine runtime/logic bugs that ship silently.
...
|---|----------|----------|-------|
| A1 | **High (functional)** | AppViews.tsx:4875 | Reports recurring data always empty (`recurring` vs `sortedRecurring`) |
| A2 | Medium (dead code) | PwaInstallPrompt.tsx:4 | Dismiss button bound to undefined `dismiss` |
| A3 | Low (cosmetic) | AppViews.tsx:6076 | Recurring `emoji` never set; always `🔁` |
| A4 | Low (dead code) | InvestmentsView.tsx | `logoUrl` fallback unreachable (should be `logo_url`) |
| B1 | Low (type) | App.tsx:65/792 | `'utilities_hub'` not in `ViewKey` |
| B2 | Low (type) | AppViews.tsx ×13 | color helper params typed too narrowly |
| B3 | Low (type) | AppViews.tsx ×4 | `replaceAll` needs `lib: ES2021` |
| B4 | Low (type) | App.tsx / AppViews.tsx | legacy matchMedia `never` |
| B5 | Low (type) | useBudgetApp.ts:1296 | recurring `kind` widened to `string` |
| B6 | Low (type) | backupService.ts | zip.js `getData` + `unknown` body |
| C1 | Medium (config) | netlify/functions/* | `@netlify/functions` undeclared dep |
| C2 | Low (config) | tsconfig.json | functions not typechecked |
| C3 | Medium (security) | vite/esbuild | dev-server audit advisory (breaking fix) |
| C4 | Low (hygiene) | .gitignore | env files not ignored |
| C5 | Info | AppViews.tsx / bundle | 1.19 MB main chunk; 9.3k-line file |

**Total:** 38 TypeScript errors (6 files) + 4 config/security notes.
The only one that changes user-visible behavior today is **A1**.
