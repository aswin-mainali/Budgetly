...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (toolsSection === 'investments' && !admin.visibleFeatures.investments) {
      ...
      else if (admin.visibleFeatures.networth) setToolsSection('networth')
    }
    if (toolsSection === 'networth' && !admin.visibleFeatures.networth) {
      if (admin.visibleFeatures.goals) setToolsSection('goals')
      else if (admin.visibleFeatures.reports) setToolsSection('reports')
      else if (admin.visibleFeatures.converter) setToolsSection('converter')
      else if (admin.visibleFeatures.investments) setToolsSection('investments')
  ...
  const handleNotificationNavigate = (target: string) => {
    ...
    if (target === 'utilities/networth') { setToolsSection('networth'); handleViewChange('tools'); return }
  ...
  const staticCommandItems: Array<Omit<CommandItem, 'group'>> = [
    ...
    { id: 'page-networth', type: 'page', label: 'Net Worth', description: 'Track assets, liabilities, and net worth over time', keywords: ['net worth', 'networth', 'assets', 'liabilities', 'wealth', 'debt', 'balance sheet', 'utilities'], onSelect: () => goToTools('networth'), iconClassName: 'green', icon: <Scale size={16} /> },
    // Settings sub-pages
        ...
        {(view === 'tools' || view === 'utilities_hub') ? (
          view === 'utilities_hub' ? (
            <section className="mobileUtilitiesHub">
              ...
              {admin.visibleFeatures.networth ? <button className="utilityCard" onClick={() => { setToolsSection('networth'); setView('tools') }}><Scale size={22} /><div><strong>Net Worth</strong><p>Track everything you own and owe over time.</p></div><ChevronRight size={18} /></button> : null}
            </section>
          ) : (
          <div className={`toolsPageShell toolsPageShellFixed ${toolsSection === 'converter' ? 'toolsShellConverter' : ''}`}>
            <div className={`toolsPageBody ${toolsSection === 'converter' ? 'toolsPageBodyConverter' : ''}`}>
              ...
              {toolsSection === 'networth' && admin.visibleFeatures.networth ? <NetWorthView currency={budget.data.currency} onOpenInvestments={admin.visibleFeatures.investments ? () => goToTools('investments') : undefined} /> : null}
            </div>
          </div>
          )
        ) : null}
