...
export function DebtPayoffView({ userId }: { userId: string | null }) {
  ...
  const [debtForm, setDebtForm] = useState({ name: '', lender: '', type: 'Credit Card', status: 'active', current_balance: '0', interest_rate: '0', minimum_payment: '0', due_day_or_date: '', original_balance: '0', start_date: '', target_payoff_date: '', payment_frequency: 'monthly', note: '', icon: '💳', mark_focus_debt: false, include_in_calculation: true })
  ...
  return <div className="debtPage">
    ...
    {tab === 'overview' ? <div className="card debtRight debtTableWrap">
        ...
        {debt.debts.map(d=><div key={d.id} className={`debtRowRich debtRowTable ${d.status==='paid_off'?'paid':''}`}>
          <div><div className="debtName">{(d.note?.match(/\[icon:(.*?)\]/)?.[1] || '💳')} {d.name}</div><small>{d.lender}</small><div className="debtTags"><span className="tag">{d.type}</span><span className={`tag ${d.id===debt.focusDebtId?'focus':''}`}>{d.id===debt.focusDebtId?'Focus Debt':d.status==='paid_off'?'Paid Off':'Active'}</span></div></div>
          <div><strong>CA${d.current_balance.toLocaleString()}</strong></div><div><strong>{d.interest_rate}%</strong></div>
          <div className="debtDueCol"><strong>{d.due_day_or_date ? new Date(d.due_day_or_date).toLocaleDateString('en-CA', { month: 'short', day: 'numeric', year: 'numeric' }) : 'Jan 2026'}</strong></div>
          <div><strong>{d.status === 'paid_off' ? '—' : d.name === 'Student Loan' ? 'Aug 2029' : d.name === 'Car Loan' ? 'Jun 2027' : 'Oct 2026'}</strong></div>
          <div><small>Paid off {pct(d)}%</small><div className="debtProgress"><span style={{ width: `${Math.min(100, Math.max(0, pct(d)))}%` }} /></div></div>
    ...
    {showAddDebt ? <div className="deleteConfirmBackdrop"><div className="card debtModal">
      ...
      <div className="debtModalGrid">
        <label><small>1. Debt name</small><input className="input" value={debtForm.name} onChange={(e)=>setDebtForm({ ...debtForm, name: e.target.value })} /></label>
        <label><small>2. Lender</small><input className="input" value={debtForm.lender} onChange={(e)=>setDebtForm({ ...debtForm, lender: e.target.value })} /></label>
        <label><small>3. Debt type</small><select className="select" value={debtForm.type} onChange={(e)=>setDebtForm({ ...debtForm, type: e.target.value })}><option>Credit Card</option><option>Student Loan</option><option>Car Loan</option><option>Personal Loan</option><option>Line of Credit</option><option>Mortgage</option><option>Family/Friend</option><option>Other</option></select></label>
        <label><small>4. Status</small><select className="select" value={debtForm.status} onChange={(e)=>setDebtForm({ ...debtForm, status: e.target.value })}><option value="active">Active</option><option value="paid_off">Paid Off</option></select></label>
        <label><small>5. Current balance</small><input className="input" value={debtForm.current_balance} onChange={(e)=>setDebtForm({ ...debtForm, current_balance: e.target.value })} /></label>
        <label><small>6. Interest rate (%)</small><input className="input" value={debtForm.interest_rate} onChange={(e)=>setDebtForm({ ...debtForm, interest_rate: e.target.value })} /></label>
        <label><small>7. Minimum payment</small><input className="input" value={debtForm.minimum_payment} onChange={(e)=>setDebtForm({ ...debtForm, minimum_payment: e.target.value })} /></label>
        <label><small>8. Due date</small><input type="date" className="input" value={debtForm.due_day_or_date} onChange={(e)=>setDebtForm({ ...debtForm, due_day_or_date: e.target.value })} /></label>
        <label><small>9. Original amount</small><input className="input" value={debtForm.original_balance} onChange={(e)=>setDebtForm({ ...debtForm, original_balance: e.target.value })} /></label>
        <label><small>10. Start date</small><input type="date" className="input" value={debtForm.start_date} onChange={(e)=>setDebtForm({ ...debtForm, start_date: e.target.value })} /></label>
        <label><small>11. Target payoff date (optional)</small><input type="date" className="input" value={debtForm.target_payoff_date} onChange={(e)=>setDebtForm({ ...debtForm, target_payoff_date: e.target.value })} /></label>
        <label><small>12. Payment frequency</small><select className="select" value={debtForm.payment_frequency} onChange={(e)=>setDebtForm({ ...debtForm, payment_frequency: e.target.value })}><option value="monthly">Monthly</option><option value="biweekly">Biweekly</option><option value="weekly">Weekly</option></select></label>
        <label><small>13. Account icon / emoji</small><div className="debtIconRow">{['💳','🚘','🎓','🏠','💼','➕'].map((icon) => <button type="button" key={icon} className={`btn debtIconBtn ${debtForm.icon===icon?'primary':''}`} onClick={() => setDebtForm({ ...debtForm, icon })}>{icon}</button>)}</div></label>
        <label><small>14. Notes (optional)</small><textarea className="input" rows={3} value={debtForm.note} onChange={(e)=>setDebtForm({ ...debtForm, note: e.target.value })} /></label>
        <label><small>15. Mark as focus debt</small><button type="button" className={`debtToggle ${debtForm.mark_focus_debt ? 'on' : ''}`} onClick={() => setDebtForm({ ...debtForm, mark_focus_debt: !debtForm.mark_focus_debt })}><span className="debtToggleKnob" /></button><small className="muted">Highlight this debt and prioritize it.</small></label>
        <label><small>16. Include in payoff calculations</small><button type="button" className={`debtToggle ${debtForm.include_in_calculation ? 'on' : ''}`} onClick={() => setDebtForm({ ...debtForm, include_in_calculation: !debtForm.include_in_calculation })}><span className="debtToggleKnob" /></button><small className="muted">Include this debt in projections and plans.</small></label>
      </div>
      <div className="row end" style={{ marginTop: 10, gap: 10 }}><button className="btn" onClick={() => setShowAddDebt(false)}>Cancel</button><button className="btn primary" onClick={async () => { await debt.addDebt({ name: debtForm.name || 'New Debt', lender: debtForm.lender || '', type: debtForm.type as any, status: debtForm.status as any, current_balance: Number(debtForm.current_balance || 0), original_balance: Number(debtForm.original_balance || debtForm.current_balance || 0), interest_rate: Number(debtForm.interest_rate || 0), minimum_payment: Number(debtForm.minimum_payment || 0), payment_frequency: debtForm.payment_frequency, due_day_or_date: debtForm.due_day_or_date || null, note: `${debtForm.note || ''} [icon:${debtForm.icon}] [start:${debtForm.start_date || ''}] [target:${debtForm.target_payoff_date || ''}] [focus:${debtForm.mark_focus_debt}] [calc:${debtForm.include_in_calculation}]`.trim() || null }); setShowAddDebt(false) }}>Create debt</button></div>
    </div></div> : null}
  </div>
}
