...
export function SafeToSpendView({ budget }: Pick<SharedProps, 'budget'>) {
  ...
  return <div className="grid safeToSpendPage" style={{ gap: 14 }}>
    <div className="safeToSpendHeader row space"><div><div className="h1 safeToSpendTitle">Safe-To-Spend</div><div className="muted safeToSpendSub">Plan how much of your remaining savings stays available for flexible spending each month. Unused amounts roll over automatically.</div></div><div className="row safeToSpendTopActions" style={{ gap: 8 }}><select className="select" value={activeMonth} onChange={(e) => setActiveMonth(e.target.value)}>{months.map((m) => <option key={m} value={m}>{helpers.monthLabel(m)}</option>)}</select><button className="btn primary" onClick={save}>Save changes</button></div></div>
    <div className="card safeToSpendPageRibbon"><div className="row space safeToSpendRibbonTop"><div className="safeToSpendHero"><span className="safeToSpendHeroIcon">🛡️</span><div><strong>{helpers.fmtMoney(available, data.currency)}</strong><div className="muted">available this month</div></div></div><div className="row safeToSpendRibbonStats" style={{ gap: 12 }}><span><i className="dot good" />{helpers.fmtMoney(allocation, data.currency)}<small>added this month</small></span><span><i className="dot info" />{helpers.fmtMoney(rollover, data.currency)}<small>rolled over</small></span><span><i className="dot warn" />{helpers.fmtMoney(spent, data.currency)}<small>spent</small></span><span><i className="dot teal" />{helpers.fmtMoney(remaining, data.currency)}<small>remaining</small></span></div><span className="badge safeToSpendDailyPill">{helpers.fmtMoney(remaining / daysLeft, data.currency)}/day left</span></div><div className="safeToSpendRibbonProgress"><div className="row space"><small>Spent vs Available</small><small>{helpers.fmtMoney(spent, data.currency)} / {helpers.fmtMoney(available, data.currency)}</small></div><div className="progress"><div style={{ width: `${available > 0 ? Math.min(100, (spent / available) * 100) : 0}%` }} /></div></div><div className="muted safeToSpendRibbonFoot">Unused amount rolls into next month.</div></div>
    <div className="grid cols2 safeToSpendMainGrid">
      <div className="card safeToSpendPanel"><h3>Setup & Allocation</h3><div className="safeToSpendGrid"><label><span>Month</span><input value={helpers.monthLabel(activeMonth)} readOnly /></label><label><span>Rollover from Last Month</span><input value={helpers.fmtMoney(rollover, data.currency)} readOnly /></label><label><span>Monthly Income</span><input value={helpers.fmtMoney(income, data.currency)} readOnly /></label><label><span>Safe-To-Spend Allocation</span><input className="safeToSpendEditable" value={draft} onChange={(e) => setDraft(e.target.value.replace(/[^\d.]/g, ''))} /></label><label><span>Monthly Expenses</span><input value={helpers.fmtMoney(expenses, data.currency)} readOnly /></label><label><span>Protected Savings</span><input value={helpers.fmtMoney(remainingSavings - allocation, data.currency)} readOnly /></label><label><span>Remaining Amount / Savings</span><input value={helpers.fmtMoney(remainingSavings, data.currency)} readOnly /></label><label><span>Total Safe-To-Spend Available</span><input className="safeToSpendTotalInput" value={helpers.fmtMoney(available, data.currency)} readOnly /></label><label style={{ gridColumn: '1 / -1' }}><span>Notes</span><textarea className="safeToSpendEditable" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Add any notes about your Safe-To-Spend plan..." /></label></div><div className="safeToSpendInfo">ⓘ Unused Safe-To-Spend will roll over into next month until spent.</div><div className="row" style={{ justifyContent: 'flex-end', gap: 8, marginTop: 10 }}><button className="btn ghost" onClick={() => { setDraft(String(saved?.allocation ?? 0)); setNotes(String(saved?.notes ?? '')) }}>Reset</button><button className="btn primary" onClick={save}>Save Setup</button></div></div>
      <div className="card"><h3>Safe-To-Spend Activity</h3><div className="safeToSpendActivityTableWrap"><table className="txTable"><thead><tr><th>Date</th><th>Type</th><th>Amount</th><th>Notes</th></tr></thead><tbody>{rows.map((r, i) => <tr key={i}><td>{new Date(`${r.date}T00:00:00`).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</td><td><span className={`badge ${r.type === 'Added' ? 'good' : 'warn'}`}>{r.type}</span></td><td className={r.type === 'Added' ? 'reportsGood' : 'reportsBad'}>{r.type === 'Added' ? '+' : '-'}{helpers.fmtMoney(Math.abs(r.amount), data.currency)}</td><td>{r.note}</td></tr>)}</tbody></table></div><PaginationControls page={page} totalPages={totalPages} onPrev={() => setPage((p) => Math.max(1, p - 1))} onNext={() => setPage((p) => Math.min(totalPages, p + 1))} /><div className="muted">Total spent from Safe-To-Spend: {helpers.fmtMoney(spent, data.currency)}</div></div>
    </div>
