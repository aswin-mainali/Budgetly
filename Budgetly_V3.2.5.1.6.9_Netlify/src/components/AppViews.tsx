...
import { getSafeToSpendSetting, migrateLocalSafeToSpendToSupabase, upsertSafeToSpendSetting } from '../lib/safeToSpendService'
...
const SAFE_TO_SPEND_CATEGORY_NAME = 'Safe to Spend'
...
export function DashboardView({ budget, theme, onOpenTransactionsByType }: Pick<SharedProps, 'budget' | 'theme' | 'onOpenTransactionsByType'>) {
  ...
  const safeToSpendTriggerRef = useRef<HTMLButtonElement | null>(null)
  ...
  const [safeToSpendDraft, setSafeToSpendDraft] = useState('')
  const [safeToSpendSavedTick, setSafeToSpendSavedTick] = useState(0)
  const safeToSpendInputRef = useRef<HTMLInputElement | null>(null)
  ...
  const parseSafeValue = (value: string | null | undefined) => {
    const parsed = Number(value ?? 0)
    return Number.isFinite(parsed) ? Math.max(0, parsed) : 0
  }
  const [safeSettingsByMonth, setSafeSettingsByMonth] = useState<Record<string, { allocation: number; notes?: string; rolloverFromLastMonth?: number; spent?: number }>>({})
  ...
  const safeToSpendModel = useMemo(() => {
    const allocation = parseSafeValue(String(safeSettingsByMonth[activeMonth]?.allocation ?? 0))
    const prevMonth = getPrevMonth(activeMonth)
    const prev = safeSettingsByMonth[prevMonth] || null
    const prevAllocation = parseSafeValue(String(prev?.allocation ?? 0))
    const prevRollover = parseSafeValue(String(prev?.rolloverFromLastMonth ?? 0))
    const rolloverFromLastMonth = Math.max(prevAllocation + prevRollover, 0)
    const remainingSavings = income - expenses
    const monthPrefix = `${activeMonth}-`
    const safeToSpendSpent = data.transactions.reduce((sum, transaction) => {
  ...
  }, [activeMonth, income, expenses, safeToSpendSavedTick, data.transactions, safeSettingsByMonth])
  ...
  useEffect(() => {
    ...
    const onEsc = (event: KeyboardEvent) => { if (event.key === 'Escape') setSafeToSpendOpen(false) }
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const monthsToLoad = Array.from(new Set([activeMonth, getPrevMonth(activeMonth)]))
      const next: Record<string, { allocation: number; notes?: string; rolloverFromLastMonth?: number; spent?: number }> = {}
  ...
  }, [activeMonth, safeToSpendSavedTick])
...
export function TransactionsView({ budget }: Pick<SharedProps, 'budget'>) {
  ...
  const filteredCategoriesForDraft = useMemo(() => {
    ...
    const scoped = preferred.length ? preferred : categories
    if (txDraft.type !== 'expense') return scoped
    const hasSafeToSpend = scoped.some((category) => category.id === SAFE_TO_SPEND_CATEGORY_ID)
    if (hasSafeToSpend) return scoped
    return [{ id: SAFE_TO_SPEND_CATEGORY_ID, name: SAFE_TO_SPEND_CATEGORY_NAME, emoji: '🛟', budget_monthly: 0 }, ...scoped]
          ...
          <div className="deleteConfirmBackdrop" role="presentation" onClick={() => setTxAddModalOpen(false)}>
            <div className="card txAddModal" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}>
              ...
              <div className="txAddModalGrid">{/* keep existing fields simple */}
                ...
                <div className="field txField"><label>Type</label><select value={txDraft.type} onChange={(event) => setTxDraft((current) => ({ ...current, type: event.target.value as TxType, category_id: event.target.value === 'expense' ? SAFE_TO_SPEND_CATEGORY_ID : '' }))}><option value="income">Income</option><option value="expense">Expense</option></select></div>
        ...
        <div className={`row gap txAddRow ${txDraft.type === 'income' ? 'incomeMode' : 'expenseMode'}`} style={{ marginTop: 12 }}>
          ...
          <div className="field txField txTypeField">
            ...
            <div className="typeToggle" role="tablist" aria-label="Transaction type">
              ...
              <button type="button" className={`typeToggleBtn expense ${txDraft.type === 'expense' ? 'active' : ''}`} onClick={() => setTxDraft((current) => ({ ...current, type: 'expense', category_id: SAFE_TO_SPEND_CATEGORY_ID }))}>Expense</button>
            </div>
          </div>
                ...
                <select
                  ...
                  onChange={(event) => {
                    ...
                    setTxDraft((current) => ({
                      ...current,
                      type: nextType,
                      category_id: nextType === 'expense' ? SAFE_TO_SPEND_CATEGORY_ID : '',
                    }))
                  }}
                >
...
export function SafeToSpendView({ budget }: Pick<SharedProps, 'budget'>) {
  ...
  const [settingsByMonth, setSettingsByMonth] = useState<Record<string, any>>({})
  const read = (m: string) => settingsByMonth[m] || null
  ...
  useEffect(() => { setDraft(String(saved?.allocation ?? 0)); setNotes(String(saved?.notes ?? '')) }, [activeMonth, saved?.allocation, saved?.notes])
  const save = () => {
  ...
  return <div className="safeToSpendPage">
    <div className="safeToSpendHeader">
      <div>
        <div className="safeToSpendTitle">Safe-To-Spend</div>
        <div className="safeToSpendSub">Plan how much of your remaining savings stays available for flexible spending each month. Unused amounts roll over automatically.</div>
      </div>
      <div className="safeToSpendHeaderActions">
        <select className="select" value={activeMonth} onChange={(e) => setActiveMonth(e.target.value)}>{months.map((m) => <option key={m} value={m}>{helpers.monthLabel(m)}</option>)}</select>
        <button className="btn primary" onClick={save}>Save changes</button>
      </div>
    </div>

    <div className="safeToSpendRibbon card">
      <div className="safeToSpendRibbonTop">
        <div className="safeToSpendRibbonLead"><span className="icon">💼</span><div><strong>{helpers.fmtMoney(available, data.currency)}</strong><small>available this month</small></div></div>
        <div className="safeToSpendRibbonStats">
          <div><em>➕</em><b>{helpers.fmtMoney(allocation, data.currency)}</b><span>added this month</span></div>
          <div><em>🔁</em><b>{helpers.fmtMoney(rollover, data.currency)}</b><span>rolled over</span></div>
          <div><em>💳</em><b>{helpers.fmtMoney(spent, data.currency)}</b><span>spent</span></div>
          <div><em>🛡️</em><b>{helpers.fmtMoney(remaining, data.currency)}</b><span>remaining</span></div>
        </div>
    ...
    <div className="safeToSpendGrid2">
      <section className="card safePanel"><h3>Setup & Allocation</h3><div className="safeToSpendGrid"><label><span>Month</span><input value={helpers.monthLabel(activeMonth)} readOnly /></label><label><span>Rollover from Last Month</span><input value={helpers.fmtMoney(rollover, data.currency)} readOnly /></label><label><span>Monthly Income</span><input value={helpers.fmtMoney(income, data.currency)} readOnly /></label><label><span>Safe-To-Spend Allocation</span><input className="safeEditable" value={draft} onChange={(e) => setDraft(e.target.value.replace(/[^\d.]/g, ''))} /></label><label><span>Monthly Expenses</span><input value={helpers.fmtMoney(expenses, data.currency)} readOnly /></label><label><span>Protected Savings</span><input value={helpers.fmtMoney(remainingSavings - allocation, data.currency)} readOnly /></label><label><span>Remaining Amount / Savings</span><input value={helpers.fmtMoney(remainingSavings, data.currency)} readOnly /></label><label><span>Total Safe-To-Spend Available</span><input className="safeTotal" value={helpers.fmtMoney(available, data.currency)} readOnly /></label><label className="full"><span>Notes</span><textarea className="safeEditable" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Add any notes about your Safe-To-Spend plan..." /></label></div><div className="safeInfo">ⓘ Unused Safe-To-Spend will roll over into next month until spent.</div><div className="safeActions"><button className="btn ghost" onClick={() => { setDraft(String(saved?.allocation ?? 0)); setNotes(String(saved?.notes ?? '')) }}>Reset</button><button className="btn primary" onClick={save}>Save Setup</button></div></section>

      <section className="card safePanel"><h3>Safe-To-Spend Activity</h3><div className="safeTableWrap"><table className="safeTable"><thead><tr><th>Date</th><th>Type</th><th>Amount</th><th>Notes</th></tr></thead><tbody>{rows.map((r, i) => <tr key={i}><td>{new Date(`${r.date}T00:00:00`).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</td><td><span className={`typePill ${r.type === 'Added' ? 'add' : 'spent'}`}>{r.type}</span></td><td className={r.type === 'Added' ? 'amtAdd' : 'amtSpent'}>{r.type === 'Added' ? '+' : '-'}{helpers.fmtMoney(Math.abs(r.amount), data.currency)}</td><td className="noteCell">{r.note}</td></tr>)}</tbody></table></div><PaginationControls page={page} totalPages={totalPages} onPrev={() => setPage((p) => Math.max(1, p - 1))} onNext={() => setPage((p) => Math.min(totalPages, p + 1))} /><div className="safeTotalSpent"><span>Total spent from Safe-To-Spend:</span><strong>{helpers.fmtMoney(spent, data.currency)}</strong></div></section>
    </div>

    <div className="safeToSpendGrid2">
      <section className="card safePanel"><h3>Rollover History</h3><p className="muted">Unused amounts continue until spent.</p><div className="safeRollList">{[2, 1, 0, -1].map((n) => { const d = new Date(Date.UTC(Number(activeMonth.slice(0, 4)), Number(activeMonth.slice(5)) - 1 - n, 1)); const m = `${d.getUTCFullYear()}-${String(d.getUTCMonth() + 1).padStart(2, '0')}`; const rr = read(m); return <div key={m} className="row space"><span>{helpers.monthLabel(m)}</span><span>{helpers.fmtMoney(Number(rr?.rolloverFromLastMonth || 0), data.currency)} {m === activeMonth ? <span className="badge">Current</span> : n < 0 ? <span className="badge">Projected</span> : null}</span></div> })}</div></section>
      <section className="card safePanel"><h3>How it works</h3><div className="safeHow"><div><span>🧮</span><div><strong>Income - Expenses = Remaining savings</strong><p>We calculate how much you have left each month.</p></div></div><div><span>🛡️</span><div><strong>Choose how much to allocate to Safe-To-Spend</strong><p>Set your monthly flexible spending allocation.</p></div></div><div><span>🔁</span><div><strong>Unused amounts roll into next month</strong><p>Your unused allocation keeps rolling over until you spend it.</p></div></div></div></section>
    </div>
  </div>
}
...
export function RecurringView({ budget }: Pick<SharedProps, 'budget'>) {
  ...
  return (
    <div className="card mobileSectionCard dataPageCard recurringFeedPage recurringDesignerPage">
      ...
      {!isPhone ? null : (
        <div className="muted" style={{ marginTop: 10 }}>Tip: tap any recurring item above to edit in the drawer.</div>
      )}
      ...
      <div className="row between recurringSummaryRow dataPageFooter" style={{ alignItems: 'center', gap: 12 }}>
        ...
        <div className="badge">Estimated monthly net {helpers.fmtMoney(monthlyNet, data.currency)}</div>
