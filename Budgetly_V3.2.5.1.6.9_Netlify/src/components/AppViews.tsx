  ...
  return <div className="debtPage">
    ...
    {showAddDebt ? <div className="deleteConfirmBackdrop"><div className="card debtModal">
      ...
      <div className="debtModalGrid">
        ...
        <label><small>Account icon / emoji</small><div className="debtIconRow">{['💳','🚘','🎓','🏠','💼','➕'].map((icon) => <button type="button" key={icon} className={`btn ${debtForm.icon===icon?'primary':''}`} onClick={() => setDebtForm({ ...debtForm, icon })}>{icon}</button>)}</div></label>
        ...
        <label><small>✅ Mark as focus debt</small><button type="button" className={`btn ${debtForm.mark_focus_debt?'primary':''}`} onClick={() => setDebtForm({ ...debtForm, mark_focus_debt: !debtForm.mark_focus_debt })}>{debtForm.mark_focus_debt ? 'Enabled' : 'Disabled'}</button></label>
        <label><small>🧮 Include in payoff calculations</small><button type="button" className={`btn ${debtForm.include_in_calculation?'primary':''}`} onClick={() => setDebtForm({ ...debtForm, include_in_calculation: !debtForm.include_in_calculation })}>{debtForm.include_in_calculation ? 'Included' : 'Excluded'}</button></label>
      </div>
