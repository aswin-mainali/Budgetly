...
export function DebtPayoffView({ userId }: { userId: string | null }) {
  ...
  return <div className="debtPage">
    ...
    {tab === 'overview' || tab === 'history' ? <div className="debtKpis">
        ...
        {debt.debts.map(d=><div key={d.id} className={`debtRowRich debtRowTable ${d.status==='paid_off'?'paid':''}`}>
          ...
          <div className="debtActions">
            ...
            {openMenuDebtId === d.id ? <div className="debtActionMenu">
              <button className="btn" onClick={() => { setCustomPayDebtId(d.id); setCustomPayMode('custom'); setCustomPayAmount(String(d.minimum_payment || 100)); setOpenMenuDebtId(null) }}>Make Payment</button>
    ...
    {showAddDebt ? <div className="deleteConfirmBackdrop"><div className="card debtModal">
      ...
      <div className="debtModalGrid">
        ...
        <label><small>Account icon / emoji</small><small className="muted">Choose an icon that best represents this account.</small><div className="debtIconRow">{['💳','🚗','🎓','🏠','💼','📱','🛍️','🐷','🏦','💰'].map((icon) => <button type="button" key={icon} className={`btn debtIconBtn ${debtForm.icon===icon?'primary':''}`} onClick={() => setDebtForm({ ...debtForm, icon })}>{icon}</button>)}</div></label>
