...
export function InvestmentsView() {
  ...
  const modalRows = useMemo<ModalSecurityRow[]>(() => {
    if (!search.trim()) return popularRows
    return suggestions.map((sec) => ({ ...sec, changeText: 'N/A', changeValue: null, logo_url: sec.logo_url || getCompanyLogoUrl(sec) || '' }))
  ...
  const openEdit = (h: Holding) => { setEditing(h); setSelected({ symbol: h.symbol, companyName: h.company_name, exchange: h.exchange || '', currency: (h.currency as 'CAD'|'USD') || 'CAD', fallbackPrice: h.current_price, logo_url: getCompanyLogoUrl({ symbol: h.symbol, logo_url: h.logo_url, domain: h.domain || resolveMeta(h.symbol).domain }) || undefined, domain: h.domain || resolveMeta(h.symbol).domain || undefined }); setQuantity(String(h.quantity)); setAverageCost(String(h.average_cost)); setAccountId(h.account_id || ''); setNote(h.notes || ''); setOpen(true) }
