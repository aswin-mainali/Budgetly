...
import { BarChart3, ListChecks, Tags, Settings, Menu, Cloud, Repeat, Headset, Wrench, Sparkles, ChevronDown, ChevronRight, Target, ArrowLeftRight, TrendingUp } from 'lucide-react'
...
export default function Sidebar(props: {
  ...
  toolsSection: 'goals' | 'reports' | 'converter' | 'debt' | 'networth'
  setToolsSection: (v: 'goals' | 'reports' | 'converter' | 'debt' | 'networth') => void
  sync: SyncState
  email?: string | null
              ...
              <div key={item.key} className="toolsNavItem">
                ...
                {toolsExpanded && !collapsed && hasAnyTool && floatingToolsPos ? createPortal(
                  <div className="toolsSubNav floating" id="utilities-submenu" style={{ position: 'fixed', top: floatingToolsPos.top, left: floatingToolsPos.left }}>
                    ...
                    <button className={toolsSection === 'networth' ? 'active' : ''} onClick={() => { setView('tools'); setToolsSection('networth'); setToolsExpanded(false) }}>
                      <TrendingUp size={16} /> <span className="navLabel">Net Worth</span>
                    </button>
                  </div>,
                  document.body
                ) : null}
